# https://www.sthda.com/english/wiki/text-mining-and-word-cloud-fundamentals-in-r-5-simple-steps-you-should-know
...
# words_2021<-read.table("./class_materials/class_sessions/01_course_intro/survey_word_cloud/words_2025.txt",sep=' ') %>% 
...
# words_2023_1<-read_csv("./class_sessions/01_course_intro/survey_word_cloud/words_2023_wk1.csv") %>%
#   mutate(year=2023) %>%
#   relocate(year,.before=1) %>%
#   mutate(words=tolower(words)) %>%
#   rename(word=words) %>%
...
words_2025<-read_csv("./class_sessions/01_course_intro/survey_word_cloud/words_2025.csv") %>% 
  # mutate(year=2025) %>% 
  # relocate(year,.before=1) %>% 
  mutate(words=tolower(words)) %>% 
  mutate(words=gsub("hot/humid","hot,humid",words)) %>% 
  separate_wider_delim(words, delim = ",", names = c("w1", "w2","w3","w4","w5","w6"),too_few = "align_start") %>% 
  mutate_all(trimws) %>% 
  pivot_longer(w1:w6,names_to="order") %>% 
  select(word=value,-order) %>% 
  # arrange(word) %>% 
  # group_by(word) %>% 
  # tally() %>% 
  # arrange(desc(n)) %>% 
  mutate(word=gsub("the ","",word)) %>% 
  mutate(word=gsub("humix","humid",word)) %>% 
  mutate(word=gsub(" :d","",word)) %>% 
  drop_na()


wordcloud <- words_2025 %>% 
  mutate(word=str_to_title(word)) %>% 
  mutate(word=gsub(" ", "", word, fixed = TRUE)) 
text<-wordcloud$word
...
fig_2025<-wordcloud_generator(docs, "week 1")
