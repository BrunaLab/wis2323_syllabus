...
# movies <- readxl::read_excel("./course_projects/movie_reviews/movies.xlsx") %>%
  movies <- read_csv(here("course_projects","movie_reviews","movies_with_rt_scores.csv")) |> 
  ...
  rename(RT=rotten_tomatoes_score,
         IMDB=imdb_score,
         MC=metacritic_score
         ) |> 
  replace_na(list(RT = "", IMDB = "",MC="", summary=""))
...
movies$IMDB <- gsub("/10", "", movies$IMDB)
movies$MC <- gsub("/100", "", movies$MC)
...
movies<-movies %>% 
  arrange(`Extra Cred.`, Year,desc(`RT`)) %>% 
